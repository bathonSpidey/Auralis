...
class PromptGenerator:
    ...
    def build_suggest_song_messages(self, user_prompt):
        return [{"role": "system", "content": self.one_song_system_prompt}, {"role": "user", "content": f"Suggest a song based on my context: {json.dumps(user_prompt)}"}]
    
    def build_playlist_messages(self, user_prompt, context):
        return [{"role": "system", "content": self.playlist_system_prompt}, {"role": "user", "content": f"{user_prompt}. A bit about myself {json.dumps(context) if context else ''}"}]
